...
class EntregarCredenciales extends Page implements HasForms, HasTable
{
    ...
    protected string $view = 'filament.pages.entregar-credenciales';
    ...
    protected function getRoleRestrictedUserId(): ?int
    {
        ...
        if (collect($user->getRoleNames())->filter(fn ($r) => str_starts_with($r, 'economía'))->isNotEmpty()) {
            return null;
        } // roles economía sin restricción
    ...
    protected function baseUnionQuery(): Builder
    {
        ...
        if ($userRestrict) {
            $doc->where('procesodocente.user_id', $userRestrict);
        }
        ...
        if ($userRestrict) {
            $adm->where('procesoadministrativo.user_id', $userRestrict);
        }
        ...
        if ($userRestrict) {
            $alu->where('procesoalumno.user_id', $userRestrict);
        }
    ...
    protected function getDistinctOptions(string $column): array
    {
        ...
        if (! $this->filters['proceso_fecha_id']) {
            return [];
        }
