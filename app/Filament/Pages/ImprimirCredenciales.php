...
class ImprimirCredenciales extends Page implements HasForms, HasTable
{
    ...
    protected string $view = 'filament.pages.imprimir-credenciales';
            ...
            ->schema([
                ...
                Select::make('proceso_id')
                    ...
                    ->afterStateUpdated(function () {
                        $this->data['proceso_fecha_id'] = null;
                        $this->clearFiltersContext();
                    })
                    ->required()
                    ->dehydrated(true),
            ...
            ->filters([
                Filter::make('solo_pendientes')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        ...
                        return match ($tipo) {
                            1 => $query->where(function ($q) {
                                $q->where(function ($w) {
                                    $w->whereNull('prodoc_iCredencial')->orWhere('prodoc_iCredencial', false);
                                });
                            }),
                            2 => $query->where(function ($q) {
                                $q->where(function ($w) {
                                    $w->whereNull('proadm_iCredencial')->orWhere('proadm_iCredencial', false);
                                });
                            }),
                            3 => $query->where(function ($q) {
                                $q->where(function ($w) {
                                    $w->whereNull('proalu_iCredencial')->orWhere('proalu_iCredencial', false);
                                });
                            }),
                            default => $query,
                        };
                    }),
                Filter::make('local_filter')
                    ->label('Local')
                    ->form([
                        Select::make('loc_iCodigo')
                            ->label('Local')
                ...
                Filter::make('cargo_filter')
                    ->label('Cargo')
                    ->form([
                        Select::make('expadm_iCodigo')
                            ->label('Cargo')
                    ...
                    ->action(function (Collection $records) {
                        ...
                        foreach ($records as $record) {
                            if ($record instanceof ProcesoDocente) {
                                $record->prodoc_iCredencial = true;
                                $record->prodoc_dtFechaImpresion = now();
                            } elseif ($record instanceof ProcesoAdministrativo) {
                                $record->proadm_iCredencial = true;
                                $record->proadm_dtFechaImpresion = now();
                            } elseif ($record instanceof ProcesoAlumno) {
                                $record->proalu_iCredencial = true;
                                $record->proalu_dtFechaImpresion = now();
                            }
                            $record->save();
                        }
    ...
    protected function isSoloPendientesFilterActive(): bool
    {
        ...
        if (method_exists($this, 'getTableFilterState')) {
            try {
                $candidates[] = $this->getTableFilterState('solo_pendientes');
    ...
    protected function handleImpresionForRecords(Collection $records)
    {
        ...
        foreach ($processRecords as $record) {
            ...
            $flagCol = null;
            $fechaCol = null;
            $credencialNumero = null;
            if ($record instanceof ProcesoDocente) {
                $dni = $record->docente?->doc_vcDni;
                $codigo = $record->docente?->doc_vcCodigo;
                $nombres = $record->docente?->nombre_completo;
                $flagCol = 'prodoc_iCredencial';
                $fechaCol = 'prodoc_dtFechaImpresion';
                $credencialNumero = $record->prodoc_iCodigo ?? null;
            } elseif ($record instanceof ProcesoAdministrativo) {
                $dni = $record->administrativo?->adm_vcDni;
                $codigo = $record->administrativo?->adm_vcCodigo;
                $nombres = $record->administrativo?->adm_vcNombres;
                $flagCol = 'proadm_iCredencial';
                $fechaCol = 'proadm_dtFechaImpresion';
                $credencialNumero = $record->proadm_iCodigo ?? null;
            } elseif ($record instanceof ProcesoAlumno) {
                $dni = $record->alumno?->alu_vcDni;
                $codigo = $record->alumno?->alu_vcCodigo;
                $nombres = $record->alumno?->nombre_completo;
                $flagCol = 'proalu_iCredencial';
                $fechaCol = 'proalu_dtFechaImpresion';
                $credencialNumero = $record->proalu_iCodigo ?? null;
            }
            ...
            if ($cargo) {
                $max = 28;
                if (mb_strlen($cargo) > $max) {
                    $words = preg_split('/\s+/u', $cargo);
                    $l1 = '';
                    foreach ($words as $w) {
                        if (mb_strlen(trim($l1.' '.$w)) <= $max) {
                            $l1 = trim($l1.' '.$w);
                        } else {
                            $rest = trim(mb_substr($cargo, mb_strlen($l1)));
                            $cargoLine1 = $l1;
                            $cargoLine2 = $rest;
                            break;
                        }
            ...
            $dniFile = $dni ? $dni.'.jpg' : null;
            $publicFoto = $dniFile && file_exists(public_path('storage/fotos/'.$dniFile)) ? public_path('storage/fotos/'.$dniFile) : public_path('storage/fotos/sinfoto.jpg');
            $items[] = [
                'dni' => $dni, 'codigo' => $codigo, 'nombres' => $nombres,
                'nombres_line1' => (function ($name) {
                    if (! $name) {
                        return null;
                    } $max = 28;
                    ...
                    foreach ($words as $w) {
                        if (mb_strlen(trim($l1.' '.$w)) <= $max) {
                            ...
                            $rest = trim(mb_substr($name, mb_strlen($l1)));
                            break;
                        }
                    }

                    return $l1 ?: mb_substr($name, 0, $max);
                })($nombres),
                'nombres_line2' => (function ($name) {
                    if (! $name) {
                        return null;
                    } $max = 28;
                    if (mb_strlen($name) <= $max) {
            ...
            if ($record instanceof ProcesoDocente) {
                $idsDoc[] = $record->getKey();
            } elseif ($record instanceof ProcesoAdministrativo) {
                $idsAdm[] = $record->getKey();
            } elseif ($record instanceof ProcesoAlumno) {
                $idsAlu[] = $record->getKey();
            }
        }

        $inicio = (int) (data_get($this->data, 'inicio_slot') ?? 1);
        $inicio = $inicio < 1 ? 1 : ($inicio > 4 ? 4 : $inicio);
        if ($inicio > 1) {
            $placeholders = array_fill(0, $inicio - 1, null);
            $items = array_merge($placeholders, $items);
        }
        ...
        $sessionKeyAnv = 'plantilla_anv_'.$procesoFechaId;
        $sessionKeyRev = 'plantilla_rev_'.$procesoFechaId;
        $cachedAnv = session()->get($sessionKeyAnv);
        $cachedRev = session()->get($sessionKeyRev);
        $anvB64 = null;
        $revB64 = null;
        ...
        try {
            ...
            Log::warning('No se pudo escribir fallback URL: '.$e->getMessage());
        }
        ...
        try {
            if (! empty($idsDoc)) {
                $docTable = (new ProcesoDocente)->getTable();
                $cols = ['prodoc_iCredencial' => true, 'prodoc_dtFechaImpresion' => now()];
                if (\Schema::hasColumn($docTable, 'user_idImpresion')) {
                    $cols['user_idImpresion'] = auth()->id();
                }
                if (\Schema::hasColumn($docTable, 'IpImpresion')) {
                    $cols['IpImpresion'] = request()->ip();
                }
                $updated += ProcesoDocente::whereIn((new ProcesoDocente)->getKeyName(), $idsDoc)->update($cols);
            }
            if (! empty($idsAdm)) {
                $admTable = (new ProcesoAdministrativo)->getTable();
                $cols = ['proadm_iCredencial' => true, 'proadm_dtFechaImpresion' => now()];
                if (\Schema::hasColumn($admTable, 'user_idImpresion')) {
                    $cols['user_idImpresion'] = auth()->id();
                }
                if (\Schema::hasColumn($admTable, 'IpImpresion')) {
                    $cols['IpImpresion'] = request()->ip();
                }
                $updated += ProcesoAdministrativo::whereIn((new ProcesoAdministrativo)->getKeyName(), $idsAdm)->update($cols);
            }
            if (! empty($idsAlu)) {
                $aluTable = (new ProcesoAlumno)->getTable();
                $cols = ['proalu_iCredencial' => true, 'proalu_dtFechaImpresion' => now()];
                if (\Schema::hasColumn($aluTable, 'user_idImpresion')) {
                    $cols['user_idImpresion'] = auth()->id();
                }
                if (\Schema::hasColumn($aluTable, 'IpImpresion')) {
                    $cols['IpImpresion'] = request()->ip();
                }
                $updated += ProcesoAlumno::whereIn((new ProcesoAlumno)->getKeyName(), $idsAlu)->update($cols);
            }
        } catch (\Throwable $e) {
        ...
        try {
            ...
            Log::warning('Error refrescando tabla: '.$e->getMessage());
        }
