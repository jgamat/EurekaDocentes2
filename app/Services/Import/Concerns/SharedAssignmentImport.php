...
trait SharedAssignmentImport
{
     ...
     * codigo,dni,paterno,materno,nombres,cargo,local,fecha,monto
    ...
    public const UNIFIED_HEADERS = [
        // Formato oficial (7 columnas):
        // codigo, dni, nombres, cargo, local, fecha, monto
    ...
    protected function parseFecha(?string $raw): ?string
    {
        ...
        foreach ($fmts as $f) {
            try {
                $c = Carbon::createFromFormat($f, $raw);
                if ($c) {
        ...
        if (is_numeric($raw)) { // Excel serial date
            try {
                return Carbon::createFromTimestamp(((int) $raw - 25569) * 86400)->format('Y-m-d');
            } catch (\Throwable $e) {
            }
        }
    ...
    {
        if (! $nombreBase || ! $importado) {
            return false;
        } // sin suficiente data no marcamos
        $base = $this->normExtended($nombreBase);
        $imp = $this->normExtended($importado);
        if (! $base || ! $imp) {
            return false;
        }
        if (str_contains($base, $imp)) {
            return false;
        } // substring directa
