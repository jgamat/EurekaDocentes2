<x-filament-panels::page>
    ...
    @livewire('asignados-administrativo-table')

    <script>
    document.addEventListener('DOMContentLoaded', function () {
        const input = document.getElementById('busqueda_administrativo');
        const hidden = document.getElementById('administrativo_dni');

        function syncDni() {
            const value = input.value;
            // Formato esperado: "NOMBRES - DNI - CODIGO"
            const match = value.match(/-\s*(\d{8})\s*-/);
            if (match) {
                hidden.value = match[1];
            } else {
                hidden.value = '';
            }
            // Dispara actualización solo en change/blur para evitar notificaciones por tecla
            hidden.dispatchEvent(new Event('input', { bubbles: true }));
            hidden.dispatchEvent(new Event('change', { bubbles: true }));
        }

        if (input && hidden) {
            input.addEventListener('change', syncDni);
            input.addEventListener('blur', syncDni);
        }
    });
    </script>
</x-filament-panels::page>
